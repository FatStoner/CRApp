...
pub fn paint_avatar_crop(ui: &mut egui::Ui, rect: egui::Rect, uri: &str, rounding: f32) {
    ...
    match ui.ctx().try_load_image(uri.into(), Default::default()) {
        Ok(poll) => {
            match poll {
                ...
                egui::load::ImagePoll::Pending { .. } => {
                    ...
                    ui.painter().rect_filled(rect, egui::Rounding::same(rounding), egui::Color32::from_gray(30));
                    ui.spinner(); 
        ...
        Err(_) => {
            ui.painter().rect_filled(rect, egui::Rounding::same(rounding), egui::Color32::from_gray(60));
